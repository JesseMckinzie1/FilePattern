...
Map Pattern::matchFilesLoop(Map& map, const string& file, const regex& patternRegex, vector<string>& parsedRegex){
    int i = 0;
    string pattern;
    string s = "";
    ...
    for(const auto& element: mapping){
        if(isdigit(element.second)){
            static_cast<ObjectInt*>(oi)->a = stoi(element.second, &sz);
            mapping[element.first] = oi;
        } else {
            static_cast<ObjectString*>(os)->a = element.second;
            mapping[element.first] = os;
        }
    }
